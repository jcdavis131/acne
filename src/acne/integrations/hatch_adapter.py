"""
Runtime adapter — native for Scout/Scout runtime agents (backward compat name: hatch_adapter).

This module gives you native tools without extra glue:

  from acne.integrations import get_runtime_tools
  tools = get_runtime_tools()   # [{name, description, parameters, execute}]
...
Extras:
- get_hatch_skill() → dict you can drop into a Skill's SKILL.md (legacy name)
- Works with everyday-language expectation (why/confidence in output)
"""
...
# Alias for new public names
get_runtime_tools = get_hatch_tools
get_scout_tools = get_hatch_tools
get_agent_tools = get_hatch_tools
